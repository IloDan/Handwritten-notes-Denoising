...
task = clearml.Task.init(project_name='Unet_Danilo_parallel', task_name='Training')
...
# Definisci lo scheduler OneCycleLR con il numero corretto di iterazioni totali
total_iterations = len(train_dataloader) * NUM_EPOCHS
scheduler = OneCycleLR(opt, max_lr=LEARNING_RATE, total_steps=total_iterations)
...
for e in range(NUM_EPOCHS):
    ...
    # Salvataggio del modello dopo ogni epoca
    torch.save(net.state_dict(), f'Unet_1024_{e+1}epochs.pth')
    print(f"Model saved at epoch {e+1}")
    task.upload_artifact(f'Unet_1024_{e+1}epochs.pth', artifact_object=f'Unet_1024_{e+1}epochs.pth')
